...
void display_string(const char * string)
{
	...
	static char remain_buffer[50000]="";
	if (strlen(remain_buffer)+strlen(string)>=50000)
	{
		my_error("Too long output line, please adjust the size of remain_buffer in function display_string in file pump.c\n");
	}
	strcat(remain_buffer,string);
	char *p_return = strrchr(remain_buffer,'\n');
	...
	if (p_return!=NULL)
	{
		*p_return='\0';
		...
		pump_strcpy(remain_buffer,p_return+1);
		use_pump_command=0;
	}
#endif
}

char *pump_strcpy(char *d, const char *s)
{
  char *r = d;
  while ((*d++ = *s++));
  return r;
